// Package testserver implements a stateful test-only Relay upstream.
package testserver
...
func (s *Server) control(w http.ResponseWriter, r *http.Request) {
	...
	if err := requireEOF(decoder); err != nil || input.DelayMS < 0 || input.DelayMS > 60000 {
		http.Error(w, "invalid", 400)
		return
	}
	s.mu.Lock()
...
// New owns its configuration, independently of the caller's maps and slices.
func New(cfg Config, policy tlsconfig.IdentityPolicy) *Server {
...
// Handler serves gRPC and protected harness controls over caller-owned TLS.
